...
# Set local directory
local_dir <- here("Fig_2D/Stat_analysis")

# Read input CSV file located in same folder as script
data <- read.csv(file.path(local_dir, "IDR3-TCF3-median-dist-poly.csv"))
data <- na.omit(data)
...
write.csv(anova_table, file.path(local_dir, "Output/anova.csv"))
...
if (anova_table$"Pr(>F)"[1] < 0.05 || anova_table$"Pr(>F)"[2] < 0.05 || anova_table$"Pr(>F)"[3] < 0.05) {
  ...
  # Write output files
  write.csv(posthoc_factor1_df, file.path(local_dir, "Output/posthoc_factor1.csv"))
  write.csv(posthoc_factor2_df, file.path(local_dir, "Output/posthoc_factor2.csv"))
  write.csv(posthoc_interaction_df, file.path(local_dir, "Output/posthoc_interaction.csv"))
}
