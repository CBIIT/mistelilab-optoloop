...
summary_tbl <- summary_tbl %>%  
  mutate(avg_spots = if_else(light == "Ctrl_1", NA_real_, avg_spots)) %>%
  mutate(avg_spots = if_else(light == "sgTERT_Light_1", NA_real_, avg_spots)) %>%
  mutate(avg_spots = if_else(light == "sgSubtel+TERT_Light_3", NA_real_, avg_spots))
```
